...
class Class_Source_Get():
    ...
    def __init__(self,input = str,cookie = str,parent = object):
        super().__init__()
        ...
        self.cookie = cookie
        self.PlayInfo_pattern = re.compile('<script>window.__playinfo__=(.*?)</script>')
    ...
    def Bascical_Info(self):
        ...
        try:
            ...
            Title = re.findall('title="(.*?)"', self.response.text)[0].replace("amp;","")
            self.Title = Title
    ...
    def Get_playinfo(self,page):
        ...
        playinfo = self.PlayInfo_pattern.findall(self.response.text)[0]
        return playinfo

    
    def Video_Download(self,Quality,downflag,playinfo):
        cdef long long downloaded_bytes
        downloaded_bytes = 0
        cdef int downloaded_percent
        cdef down_description
        cdef list download_info
            ...
            for i in range(1,20):
                ...
                with open(self.video_temp_name, "ab",) as file:
                    for data in video_res.iter_content(chunk_size=32*1024):
                        ...
                        down_description = f"{self.Title[:15]}...视频已下载："
                        download_info = [down_description,downloaded_percent,0,downflag]
        ...
        try:
            ...
            with open(self.audio_temp_name, "wb") as file:
                ...
                for data in audio_res.iter_content(chunk_size=32*1024):
                    ...
                    down_description = f"{self.Title[:15]}...音频已下载："
                    download_info = [down_description,downloaded_percent,0,downflag]
                    self.parent.parent.VideoDownload_thread[downflag].down_percent_sig.emit(download_info)
                    file.write(data)
            Errorcode = 0
